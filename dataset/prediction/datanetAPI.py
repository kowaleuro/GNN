    ...
    def __iter__(self):
        ...
        for root, file in tuple_files:
            try:
                ...
                dir_info = file[0:-7]
                results_file = tar.extractfile(dir_info+"/experimentResults.txt")
                input_files = tar.extractfile(dir_info+"/input_files.txt")
                if (dir_info+"/QoSexperimentResults.txt" in tar.getnames()):
                    qosresults_file = tar.extractfile(dir_info+"/QoSexperimentResults.txt")
                else:
                    qosresults_file = None
                if (dir_info+"/experimentResultsFlows.txt" in tar.getnames()):
                    flow_results_file = tar.extractfile(dir_info+"/experimentResultsFlows.txt")
                else:
                ...
                while(True):
                    try:
                        ...
                        _tm_file = tar.extractfile(dir_info+"/tm/tm-"+num_it+".txt")
        ...
        for i in range(0,len(r), int(math.sqrt(len(r)))):
            ...
            for j in range(i, i+int(math.sqrt(len(r)))):
                ...
                try:
                    ...
                    metrics = list(map(float, metrics_[:-1]))
                    metrics.append(0.0)
